  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
          <MetricCard title="Ingest" value={`${metrics?.ingest_ms ?? 0}ms`} icon={<Timer className="h-4 w-4" />} loading={loading} />
          <MetricCard title="Plan" value={`${metrics?.plan_ms ?? 0}ms`} icon={<Timer className="h-4 w-4" />} loading={loading} />
          <MetricCard title="Apply" value={`${metrics?.apply_ms ?? 0}ms`} icon={<Timer className="h-4 w-4" />} loading={loading} />
          <MetricCard title="DB" value={`${metrics?.db_ms ?? 0}ms`} icon={<Database className="h-4 w-4" />} loading={loading} />
          <MetricCard title="Cache Hit" value={`${((metrics?.cache_hit_rate ?? 0) * 100).toFixed(1)}%`} icon={<Zap className="h-4 w-4" />} loading={loading} />
          <div className="rounded-lg border bg-card p-4 flex items-center justify-center">
            {loading ? (
              <div className="h-8 w-16 rounded bg-muted animate-pulse" />
            ) : (
              <StatusBadge
                label={`Regression: ${metrics?.regression_status ?? "UNKNOWN"}`}
                severity={metrics?.regression_status === "PASS" ? "success" : metrics?.regression_status === "FAIL" ? "destructive" : "caution"}
                dot
              />
            )}
          </div>
