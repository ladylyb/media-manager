...
const operations: OpConfig[] = [
  {
    ...
    execute: (p) => runIngest({ root_path: p.root_path || undefined }),
  },
  {
    ...
    mutating: false, icon: <Map className="h-5 w-5" />, execute: () => runPlan(),
  },
  {
    id: "apply", title: "Apply", description: "Apply the most recent plan, executing file moves and ledger updates. Irreversible.",
    mutating: true, icon: <CheckSquare className="h-5 w-5" />, execute: () => runApply(),
  },
  {
    ...
    mutating: true, icon: <RefreshCw className="h-5 w-5" />, execute: () => runCanonicalRecompute(),
  },
  {
    id: "tag-enrichment", title: "Tag Enrichment", description: "Run AI-based tag enrichment on untagged canonical files.",
    mutating: true, icon: <Tag className="h-5 w-5" />, execute: () => runTagEnrichment(),
  },
  {
    ...
    inputs: [{ key: "root_path", label: "Root Path", placeholder: "/media/incoming" }],
    execute: (p) => runOperatorRun({ root_path: p.root_path || undefined }),
  },
];
